...
export default function Entry() {
  ...
  let tagsData = JSON.parse(decodeURIComponent(tags))
  return (
    <>
      <MyHead title={entry} />
      <div id='top'></div>
      <div className="p-5 lg:px-48 2xl:px-96 lg:py-10 2xl:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <Link href={`/assets/${entry}.${format}`}>
            <Image className="rounded object-contain align-middle w-full h-full" width={1000} height={1000} src={`/assets/${entry}.${format}`} alt={entry}/>
          </Link>
          <div className="flex flex-col gap-2 flex-shrink">
            <div className="text-2xl xl:text-4xl text-primary pt-4 xl:pt-10">File name</div>
            <Link href={`/assets/${entry}.${format}`} className="text-lg xl:text-2xl text-primary pb-8 xl:pb-20">{entry}.{format}</Link>
